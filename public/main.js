...
/***/ function(module, exports, __webpack_require__) {
	...
	var SearchBar = __webpack_require__(1);
	var SliderBlock = __webpack_require__(2);
	var AcceptedSet = __webpack_require__(3);
	...
	var Suggestion = function Suggestion(props) {
	    ...
	    return React.createElement(
	        'button',
	        { type: 'button', className: 'list-group-item suggestion', onClick: onClick },
	        React.createElement(
	            'span',
	            { className: 'badge' },
	            props.idx
	...
	var GroupSelectTool = React.createClass({
	    ...
	    calcRelatedScores: function calcRelatedScores() {
	        var _app = this;
	        _app.relatedPool.forEach(function (el) {
	            el.score = el.lastUse / _app.lastUseIdx * _app.recentWeight + el.count / _app.mostUsed * _app.mostWeight + el.assocs / _app.mostRelated * _app.relatedWeight;
	        });
	    },

	    calcNonRelatedScores: function calcNonRelatedScores() {
	        var _app = this;
	        _app.suggestionPool.forEach(function (el) {
	            el.score = el.lastUse / _app.lastUseIdx * _app.recentWeight + el.count / _app.mostUsed * _app.mostWeight;
	        });
	    },

	    updateWeights: function updateWeights(related, recent, most) {
	    ...
	    combineLists: function combineLists() {
	        var _this = this;

	        return this.relatedPool.concat(this.suggestionPool).filter(function (el) {
	    ...
	    onTextChange: function onTextChange(text) {
	        ...
	        var tempSuggestions = this.suggestionPool.filter(function (el, idx, arr) {
	            return regEx.test(el.number);
	        }).slice(0, this.suggestionCount);

	        var finish = function finish() {
	            // Sort the pool by score
	            _this2.suggestionPool.sort(function (a, b) {
	                return b.score - a.score;
	            });
	            ...
	            if (this.depletedResults) {
	                ...
	                if (deficit) {
	                    ...
	                    Service.getLeases({
	                        ...
	                        _this2.depletedResults = data.length < deficit;

	                        if (data.length) {

	                            // Add the new data to the suggestions and the suggestion pool
	                            tempSuggestions = tempSuggestions.concat(data);
	                            _this2.suggestionPool = _this2.suggestionPool.concat(data);

	                            // Remove extra suggestions from the pool
	                            var tooMany = Math.min(_this2.poolMax - _this2.suggestionPool.length, 0);
	                            if (tooMany) {
	                                _this2.suggestionPool.splice(tooMany);
	                            }
	                        }
	                        finish();
	                    });
	    ...
	    cleanSuggestions: function cleanSuggestions() {
	        ...
	        this.suggestionPool = this.suggestionPool.filter(function (suggestion) {
	            return !_this3.state.accepteds.some(function (accept) {
	    ...
	    acceptLease: function acceptLease(text) {
	        var that = this;
	        var set = text.split('\n');
	    ...
	    toggleStale: function toggleStale(idx) {
	    ...
	    getRelations: function getRelations() {
	        ...
	        var filteredByStale = this.state.accepteds.filter(function (el) {
	            return !el.stale;
	        });
	        if (filteredByStale.length) {
	            Service.getRelatedData({
	                list: filteredByStale.map(function (el) {
	                    return el.number;
	                }),
	                limit: this.suggestionCount
	            }, function (data) {
	                _this4.relatedPool = data;
	                fin();
	            });
	        } else {
	            this.relatedPool = [];
	    ...
	    commit: function commit() {
	        ...
	        Service.doPOST("/commit", {
	            'list': this.state.accepteds.map(function (el) {
	                return el.number;
	            })
	        }, function (data) {

	            // Mark all leases as stale
	            _this5.state.accepteds.forEach(function (el) {
	                el.stale = true;
	            });
	            _this5.forceUpdate();

	            Service.getMaxValues({}, function (data) {
	                _this5.lastUseIdx = data.lastUseIdx;
	                _this5.mostUsed = data.mostUsed;
	            });
	        });
	    },
	    ...
	    updateOmissions: function updateOmissions() {
	        // We don't want anything from the accepted list or the suggestion pool;
	        this.omissions = this.suggestionPool.map(function (el) {
	            return el.number;
	        });
	        this.omissions = this.omissions.concat(this.state.accepteds.map(function (el) {
	            return el.number;
	        }));
	    },
	    ...
	    componentDidMount: function componentDidMount() {
	        ...
	        Service.getLeases({
	            ...
	            _this6.suggestionPool = data;
	            _this6.setState({
	                suggestionList: _this6.suggestionPool.slice(0, _this6.suggestionCount)
	            });
	        });
	        this.getMaxValues();
	        //document.addEventListener('keydown',this.handleHotkey,false);
	    },
	    getMaxValues: function getMaxValues() {
	        var _this7 = this;

	        Service.getMaxValues({}, function (data) {
	            _this7.lastUseIdx = data.lastUseIdx;
	            _this7.mostUsed = data.mostUsed;
	        });
	    },
	    render: function render() {
	        ...
	        var suggestions = this.state.suggestionList.map(function (result, i) {
	            return React.createElement(Suggestion, { key: i, data: result, idx: i + 1, acceptLease: _this8.acceptLease });
	        });
	        return React.createElement(
	            'div',
	            { className: 'app' },
	            ...
	            React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    !this.state.suggestionList.length && React.createElement(
	                        'h2',
	                        null,
	                        'No Matches'
	                    )
	            ...
	            React.createElement(AcceptedSet, {
...
/***/ function(module, exports) {
	...
	module.exports = React.createClass({
	    ...
	    onAdjust: function onAdjust(e) {
	        var _this = this;

	        clearTimeout(this.timeoutID);
	        this.state[e.target.id] = parseInt(e.target.value);
	        this.forceUpdate(function () {
	            _this.timeoutID = setTimeout(_this.props.updateWeights, 150, _this.state.relations, _this.state.recent, _this.state.totalUse);
	        });
	    },
	    render: function render() {
	        return React.createElement(
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ClipboardBtn = __webpack_require__(4);

	var Acceptance = function Acceptance(props) {
	    var toggleStale = function toggleStale(e) {
	    ...
	    return React.createElement(
	        ...
	        React.createElement(
	            "div",
	            { className: props.data.stale ? "stale" : "fresh", onClick: toggleStale, id: props.id },
	            props.data.number
	        )
	    );
	};
	...
	module.exports = function (props) {
	    var copytext = props.data.filter(function (el) {
	        return !el.stale;
	    }).map(function (el) {
	        return el.number;
	    }).join('\n');
	    var enableButton = props.data.some(function (el) {
	        return !el.stale;
	    });
	    return React.createElement(
	        "div",
	        ...
	        React.createElement(
	            "ol",
	            null,
	            props.data.map(function (accept, i) {
...
/***/ function(module, exports) {
	...
	var doPOST = function doPOST(url, options, callback) {
	    $.ajax({
	        type: "POST",
	        url: url,
	        data: options,
	...
	var doGET = function doGET(url, options, callback) {
	    $.ajax({
	        type: "GET",
	        url: url,
	        data: options,
	...
	var getRelatedData = function getRelatedData(options, callback) {
	    doGET("/getRelated", options, callback);
	};

	var getLeases = function getLeases(options, callback) {
	    doGET("/getLeases", options, callback);
	};

	var getMaxValues = function getMaxValues(options, callback) {
